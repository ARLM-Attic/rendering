    ...
    public class ILReader
    {
        ...
        int NextBlockStart(List<ILInstruction> instructions, int start)
        {
            do
            {
                ...
                if (start == -1)
                    return -1;
                if (instructions[start + 1].OpCode == OpCodes.Stfld && ((ShaderField)instructions[start + 1].Operand).Name == "<>1__state")
                    return start + 2;
                start++;
            } while (true);
        }
        int NextBlockEnd(List<ILInstruction> instructions, int start)
        {
            do
            {
                ...
                if (start == -1)
                    return -1;
                if (((ShaderField)instructions[start].Operand).Name == "<>2__current")
                    return start - 1;
                start++;
            } while (true);
        }

        public IEnumerable<ILInstruction> GetILInvolvedOnLeazyMethod(MethodInfo method)
        {
        ...
        public IEnumerable<ILInstruction> GetILForLeazyMethod(MethodInfo method)
        {
